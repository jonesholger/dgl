...
CMAKE_FLAGS="$CMAKE_FLAGS -DCMAKE_INSTALL_RPATH=$TORCH_BASE/lib:$TORCH_BASE -DCMAKE_BUILD_RPATH=$TORCH_BASE/lib:$TORCH_BASE" 
...
#		$CMAKE_COMMAND $CMAKE_FLAGS -DCMAKE_EXE_LINKER_FLAGS="-L $HIP_LIBRARIES_BASE/lib " -DPYTHON_INTERP=$PYTHON_INTERP ../..
		$CMAKE_COMMAND $CMAKE_FLAGS -DCMAKE_EXE_LINKER_FLAGS="-L $TORCH_BASE/lib " -DPYTHON_INTERP=$PYTHON_INTERP ../..
		make -j
		cp -v $CPSOURCE $BINDIR/tensoradapter/pytorch
		cd ..
	done
