...
//#if CUDART_VERSION >= 10010
  ...
  HIPSPARSE_CALL(hipsparseCsr2cscEx2_bufferSize(
      thr_entry->hipsparse_handle, csr.num_rows, csr.num_cols, nnz, data_ptr,
      indptr_ptr, indices_ptr, t_data_ptr, t_indptr_ptr, t_indices_ptr,
      HIPBLAS_R_32F, HIPSPARSE_ACTION_NUMERIC, HIPSPARSE_INDEX_BASE_ZERO,
      HIPSPARSE_CSR2CSC_ALG1,  // see cusparse doc for reference
      &workspace_size));
  void* workspace = device->AllocWorkspace(ctx, workspace_size);
  HIPSPARSE_CALL(hipsparseCsr2cscEx2(
      thr_entry->hipsparse_handle, csr.num_rows, csr.num_cols, nnz, data_ptr,
      indptr_ptr, indices_ptr, t_data_ptr, t_indptr_ptr, t_indices_ptr,
      HIPBLAS_R_32F, HIPSPARSE_ACTION_NUMERIC, HIPSPARSE_INDEX_BASE_ZERO,
      HIPSPARSE_CSR2CSC_ALG1,  // see cusparse doc for reference
...
#else
  HIPSPARSE_CALL(hipsparseScsr2csc(
